...
export const prepareCashFlowData = (stockDataToUpdate) => {
  return stockDataToUpdate.map((stockInfo, i) => {
    const isTtm = i === 10;
    const isTtmLastYear =
      i === 10 && stockInfo.revenue === stockDataToUpdate[9].revenue;
    const prevIndex = isTtmLastYear ? i - 1 : i;
    const workingCapital = isTtm
      ? calculateWorkingCapital(
          stockDataToUpdate[prevIndex].accounts_receivable,
          stockDataToUpdate[prevIndex].inventories,
    ...
    const FCFE = calculateRealFcf(stockInfo);
    const FCFF = calculateFCFF(stockInfo);
    ...
    return {
      ...
      reinvestment_rate: isFinite(
        getReinvestMentRate(i, stockDataToUpdate, FCFE)
      )
        ? getReinvestMentRate(i, stockDataToUpdate, FCFE)
        : 0,
