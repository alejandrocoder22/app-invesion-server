    ...
    for (const stock of allNonUsaStocks) {
      if (!stock.yahoo) {
        ...
        try {
          ...
          if (!response.ok) {
            throw new Error(
              `Error en la solicitud para ${stock.ticker}: ${response.status}`
            );
          }
          ...
          if (!data["Global Quote"] || !data["Global Quote"]["05. price"]) {
            await pool.query(
              ...
              [
                stock.ticker,
                "No se encontró precio válido",
                JSON.stringify(data),
              ]
            );
            console.log(
              `No se encontró precio válido para ${stock.ticker}. Respuesta de la API:`,
              data
            );
            continue;
          }
          ...
          console.log(
            `Error procesando ${stock.ticker}: ${innerError.message}`
          );
...
export const getForexPrices = async () => {
  ...
  for (const s of symbols) {
    ...
    const data = await response.json();
    try {
      await query("UPDATE forex SET price = $1 WHERE symbol = $2", [
        data[0].price,
        data[0].symbol,
      ]);
    } catch (error) {
...
export const getAllPrices = async () => {
  try {
    ...
    const filteredTickers = allTickers.filter(
      (ticker) => !nonApiTickers.has(ticker)
    );
...
export const getYahooPrice = async () => {
  const yahooStocks = allNonUsaStocks.filter((s) => s.yahoo);
  const batchSize = 15;
  const gbpTickers = ["AHT", "HLMA"];

  let gbpUsdRate = 1;
  try {
    const res = await fetch("https://api.invesion.com/stocks/forex");
    const { data } = await res.json();
    gbpUsdRate = Number(data.find((i) => i.symbol === "GBPUSD")?.price) || 1;
  } catch {}

  for (let i = 0; i < yahooStocks.length; i += batchSize) {
    const batch = yahooStocks.slice(i, i + batchSize);
    const symbols = batch.map((s) => s.symbol);

    try {
      const quotesArray = await yahoo.quote(symbols); // ¡Array!

      // Convertir a objeto {symbol: quote}
      const quotes = quotesArray.reduce((acc, q) => {
        acc[q.symbol] = q;
        return acc;
      }, {});
      ...
      for (const stock of batch) {
        ...
        if (!quote) {
          console.warn(`${stock.symbol} sin quote`);
          continue;
        }
        ...
        if (!price) {
          console.warn(`${stock.symbol} sin precio disponible`);
          continue;
        }

        let finalPrice = Number(price);
        if (gbpTickers.includes(stock.ticker)) {
          finalPrice = (finalPrice / 100) * gbpUsdRate;
        }
        ...
        console.log(`${stock.ticker} (${stock.symbol}): ${finalPrice}`);
      }
    } catch (error) {
      console.error(`Batch ${i}:`, error.message);
    }

    if (i + batchSize < yahooStocks.length) {
...
export const getYahooCurrencies = async () => {
  const currencies = [{ ticker: "CHFUSD", symbol: "CHFUSD=X" }];

  for (const s of currencies) {
    try {
      const quote = await yahoo.quote(s.symbol); // ¡Objeto directo para 1 symbol!
      ...
      console.log(`${s.ticker}: Price: ${price} ${quote.currency || "USD"}`);

      await pool.query("UPDATE forex SET price = $1 WHERE symbol = $2", [
        Number(price),
        s.ticker,
      ]);
    } catch (error) {
